...
class CarDisplay:
    def pre_sim_display(cars):
        # While the current logic of the code prevents the array from being empty when this function is called adding a check if usage changes in future
        if not cars:
    ...
    def post_sim_display(cars):
        ...
        if not cars:
            ...
            # Here I decided to report all collisions twice, once for each car
            for car in cars:
